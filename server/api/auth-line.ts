...
export default defineEventHandler(async (event) => {
    ...
    const profile: LineUserProfileResponse = await getLineUserInfo(token.access_token)
    const updateOperation = await db.collection("users").doc(event.context.userID).update({
